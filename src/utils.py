...
def parse_args(result_path):
    ...
    parser.add_argument('--method', type=str, required=True, help='Method for postprocessing, it can be either empirical or percentual.')
    args = parser.parse_args()
    args.step = None
    args.output_path = result_path
    ...
    if args.method == 'empirical':
        if args.test:
            ...
            result_path = os.path.join(result_path,args.name)
            if os.path.exists(result_path):
        ...
        print("Invalid method. Choose between empirical or percentual.")
        exit(0)
    return args
