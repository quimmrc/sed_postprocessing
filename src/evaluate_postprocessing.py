...
def best_thresholds(f1_folder, class_names, thresholds_folder):
    ...
    best_result_per_class = {c: {'f1':0, 'method':None,'thr':0} for c in class_names}
    f1_global = (None, 0)
            ...
            for scores in os.listdir(kernel_folder):
                ...
                for (label, f1) in scores_dict.items():
                    if label == 'Macro':
                        ...
                        thr_file_path = os.path.join(thresholds_folder,f'class_thresholds_{method[:-2]}.json')
    ...
    print(kernels_postprocessing)
    best_kernel = max(kernels_postprocessing, key=kernels_postprocessing.get)
    report = {'Best Kernel size for postprocessing': (best_kernel, float(kernels_postprocessing[best_kernel])),
               'Best Kernel sizes distribution': kernel_freq,
